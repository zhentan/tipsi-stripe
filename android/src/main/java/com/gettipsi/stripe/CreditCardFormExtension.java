...
/** duplicated CreditCardfForm class to make zipcode configurable **/
public class CreditCardFormExtension extends RelativeLayout {

  private CreditCardEntry entry;
  private boolean includeExp = true;
  private boolean includeSecurity = true;
  private boolean includeZip = false;
  ...
  public CreditCardFormExtension(Context context, AttributeSet attrs) {
    ...
    if (!isInEditMode()) {
      ...
      if (attrs != null) {
        ...
        try {
          ...
          this.includeZip =  typedArray.getBoolean(R.styleable.CreditCardForm_include_zip, false);
      ...
      if (cardNumberHint == null)
        cardNumberHint = context.getResources().getString(R.string.CreditCardFormCardNumberHint);
  ...
  public void setIncludeZip(final ThemedReactContext reactContext, boolean enable) {
    XmlPullParser parser = reactContext.getResources().getXml(com.gettipsi.stripe.R.xml.stub_material);
    try {
      parser.next();
      parser.nextTag();
    } catch (Exception e) {
    ...
    if (includeHelper) {
      this.removeView(textHelp);
    }
    this.removeView(currentLayout);
    includeZip = enable;
